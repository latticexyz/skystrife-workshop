...
1. Create a new namespace on an existing Sky Strife World with an `InLeague` table.
2. Add and remove players from `InLeague` with a system.
3. Allow using the system on the client.

## 1.1. Creating the table

The `contracts` folder has a `mud.config.ts` file. This is where we specifiy our custom namespace and define the tables for our project. 

We start by defining the table `InLeague` which determines which players are in the league. 

<CollapseCode>

```tsx filename="mud.config.ts" {6-9} copy showLineNumbers
...
Now let's display our league on the client.

<CollapseCode>

```tsx filename="App.tsx" {16-39, 76} copy showLineNumbers
...
Oh no, the league is empty! Let's interact with the system to use it. Note the prefix because this is a non-root system.

<CollapseCode>

```tsx filename="App.tsx" {1, 22, 30-44} copy showLineNumbers
