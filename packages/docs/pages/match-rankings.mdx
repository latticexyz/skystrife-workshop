...
1. Display the rankings for Sky Strife matches.
2. Render player addresses as human readable usernames.

## 1.1. Display match rankings

All of the tables, their types, their schemas, and other types of information for Sky Strife can be found in `mud.config.ts` in the `skystrife-contracts` folder. The client automatically synchronises these tables from the world specified in `worlds.json`.

Our alternative client will use the `MatchRanking` table. This table is set when matches finish and maps each match to an array of players (where the zeroth element is the winner of the match, the first element is 2nd place etc.).

<CollapseCode>

```ts filename="mud.config.ts" {662-664} copy showLineNumbers
...
Now we know which table we need, let's render the rankings. 

In `Leaderboard.tsx`, we fetch the records with the `useStore` hook then map over each match and player list.

<CollapseCode>

```tsx filename="Leaderboard.tsx" {31, 36-43} copy showLineNumbers
...
## 1.2. Render player names

The `bytes32` player keys in the ranking data are not super readable. Luckily, Sky Strife has a `Name` table where users can set a `string` username. 
 
We also have have to fetch the `OwnedBy` value of the entities in `MatchRanking`. This is because the ranking entries correspond to "player" entities that are “owned” by users. 

<CollapseCode>

```ts filename="mud.config.ts" {310-316, 484} copy showLineNumbers
import { mudConfig, resolveTableId } from "@latticexyz/world/register";
...
Using the `Name` table, we can conditionally render the username for a given entity in `Player.tsx`. 

<CollapseCode>

```tsx filename="Player.tsx" {1-4, 7-29} copy showLineNumbers
...
With the `OwnedBy` table, we can update `Leaderboard.tsx` fetch and display the owners of each ranking entity.

<CollapseCode>

```tsx filename="Leaderboard.tsx" {3, 10-31, 41} copy showLineNumbers
import { useMUD } from "./MUDContext";
...
We now have a list of matches and usernames. The dashboard should now look something like this:

![A picture of the rankings](./step-1.png)

Now that we have read the Sky Strife world, let's extend it!